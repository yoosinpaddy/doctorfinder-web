    ...
    public function mobileuserlogin($uname,$pass,$email)
    {
        ...
        else
        {
            ...
            if ($count)
            {
                ...
                $stmtx = $db->prepare("Select * from clinic_profile where (email=:username OR name=:username) LIMIT 1");
                $stmtx->bindParam("username", $uname, PDO::PARAM_STR);
                $stmtx->bindParam("password", $pass, PDO::PARAM_STR);
                $stmtx->execute();
                
                 $stmt1x = $db->prepare("UPDATE clinic_users set reg_id = :reg_id where (email=:username OR username=:username)");
                $stmt1x->bindParam("username", $uname, PDO::PARAM_STR);
                $stmt1x->bindParam("reg_id", $reg_id, PDO::PARAM_STR);
                $stmt1x->execute();
                
                
                $count = $stmtx->rowCount();
                $datax = $stmtx->fetch(PDO::FETCH_OBJ);
                if($data->reg_type == "appuser"){
                    $images=$datax->image;
                }
                else
                ...
                $arr=array("id"=>$data->id,"username"=>$datax->name,"email"=>$datax->email,"image"=>"non.png","created_at"=>"2020-01-01 00:00:00");
                if ($count)
                {
                    return $arr;
                }
